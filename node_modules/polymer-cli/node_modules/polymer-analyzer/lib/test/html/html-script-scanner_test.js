...
suite('HtmlScriptScanner', () => {
    ...
    `;
        ...
        chai_1.assert.isAtLeast(warnings.length, 2);
        chai_1.assert.isTrue(warnings.some((w) => w.code === 'could-not-load' &&
            !!w.message.match('does-not-exist-but-should.js')));
        chai_1.assert.isTrue(warnings.some((w) => w.code === 'not-loadable' &&
